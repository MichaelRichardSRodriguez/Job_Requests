...
namespace Job_Requests.Configurations
{
    public class JobRequestConfig : IEntityTypeConfiguration<JobRequest>
    {
        public void Configure(EntityTypeBuilder<JobRequest> builder)
        {
            ...
            builder.HasOne(c => c.JobType).WithMany(j => j.JobRequests).HasForeignKey(j => j.JobTypeId);
            builder.HasOne(c => c.JobPriority).WithMany(j => j.JobRequests).HasForeignKey(j => j.JobPriorityId);
